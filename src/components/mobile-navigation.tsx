...
export function MobileNavigation() {
  ...
  return (
    <nav
      ...
      className="mobile-bottom-nav fixed bottom-0 left-0 right-0 z-40 grid grid-cols-4 border-t md:hidden"
    >
      {items.map(([label, href, Icon]) => (
        <Link
          ...
          className={`mobile-nav-link ${active(href) ? "mobile-nav-active" : ""}`}
        >
          <Icon size={20} />
          <span>{label}</span>
        </Link>
