...
import { ArrowRight, Brain, Check, Lightbulb, Sparkles, X } from "lucide-react";
...
export function LessonRunner({ lessonId, disciplineId, topicId, title, content }: { lessonId: string; disciplineId: string; topicId: string | null; title: string; content: LessonContent }) {
  ...
  return <form action={completeMicroLesson} className="card overflow-hidden">
    ...
    <div className="h-2 bg-[var(--brand)]"/><div className="p-5 md:p-8">
      ...
      {step === 0 && <section><span className="mb-4 grid h-11 w-11 place-items-center rounded-xl bg-[var(--accent)]"><Lightbulb size={22}/></span><p className="label">Entenda o conceito</p><p className="text-lg font-semibold leading-8 whitespace-pre-wrap">{content.explanation}</p><button type="button" className="btn btn-primary mt-8 w-full" onClick={advance}>Ver um exemplo <ArrowRight size={18}/></button></section>}
      ...
      {topicId && <button formAction={createDifficulty} formNoValidate className="muted mx-auto mt-7 flex items-center gap-2 text-sm font-semibold"><Brain size={17}/>Não entendi esta parte</button>}
    </div>
  </form>;
}
