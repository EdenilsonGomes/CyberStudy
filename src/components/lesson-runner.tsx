...
function legacyCards(content: LessonContent, title: string, objective: string): LessonLearningCard[] {
  const explanation = splitIntoShortBlocks(content.explanation, 300);
  const examples = splitIntoShortBlocks(content.example, 300);
  return [
    ...explanation.map((body, index) => ({
      id: `concept-${index + 1}`,
      type: index === 0 ? "CONCEPT" as const : "ANALOGY" as const,
      title: index === 0 ? objective || title : "Observe por outro ângulo",
      eyebrow: index === 0 ? "Ideia principal" : "Pense assim",
      body,
      emoji: index === 0 ? "💡" : "🔎",
    })),
    ...examples.map((body, index) => ({ id: `scenario-${index + 1}`, type: "SCENARIO" as const, title: "Veja na prática", eyebrow: "Exemplo", body, emoji: "🧩" })),
  ];
}
...
export function LessonRunner({ lessonId, disciplineId, topicId, title, objective, contextLabel, backHref, content }: LessonRunnerProps) {
  ...
  const advance = () => { setChecked(false); setSelectedItem(0); setStep((current) => Math.min(current + 1, totalSteps - 1)); window.scrollTo({ top: 0, behavior: "smooth" }); };
  ...
  const reviewConcept = () => { setChecked(false); setSelectedItem(0); setStep(0); window.scrollTo({ top: 0, behavior: "smooth" }); };
