...
export async function PilotProgress() {
  const [row] = await getDb().select().from(interactiveSessions).where(and(eq(interactiveSessions.lessonKey, binaryPilot.id), eq(interactiveSessions.contentVersion, binaryPilot.version), isNotNull(interactiveSessions.completedAt))).orderBy(desc(interactiveSessions.completedAt)).limit(1);
  if (!row) return null;
