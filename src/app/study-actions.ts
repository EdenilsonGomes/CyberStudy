...
export async function startMaterialStudy(form: FormData) {
  ...
  const db = getDb();
  let [pack] = await db.insert(studyPackages).values({ cacheKey: target.cacheKey, kind: target.diagnostic ? "diagnostic" : "study", disciplineId: target.disciplineId, topicId: target.topicId, lessonId: target.lessonId }).onConflictDoNothing().returning();
  let ownsGeneration = Boolean(pack);
  if (!pack) {
    [pack] = await db.select().from(studyPackages).where(eq(studyPackages.cacheKey, target.cacheKey)).limit(1);
    if (!pack.content) {
      const [claimed] = await db.update(studyPackages).set({ error: null, updatedAt: new Date() }).where(and(eq(studyPackages.id, pack.id), isNull(studyPackages.content), or(eq(studyPackages.error, "generation"), lt(studyPackages.updatedAt, new Date(Date.now() - 180_000))))).returning();
  ...
  if (!pack.content && ownsGeneration) {
    try {
      ...
      [pack] = await db.update(studyPackages).set({ content, error: null, updatedAt: new Date() }).where(eq(studyPackages.id, pack.id)).returning();
    } catch (error) {
  ...
  const [created] = await db.insert(interactiveSessions).values({ lessonKey: target.key, packageId: pack.id, activeKey: target.key, contentVersion: 1, state: initialLessonState(), level }).onConflictDoNothing().returning();
...
export async function runStudyCommand(sessionId: string, command: LessonCommand) {
  ...
  try {
    const result = await getDb().transaction(async (tx) => {
      const [row] = await tx.select().from(interactiveSessions).where(eq(interactiveSessions.id, sessionId)).for("update");
      if (!row?.packageId) throw new Error("SESSION_NOT_FOUND");
      const [pack] = await tx.select().from(studyPackages).where(eq(studyPackages.id, row.packageId)).limit(1);
      ...
      if (next !== row.state) {
        await tx.update(interactiveSessions).set({ state: next, updatedAt: new Date(), completedAt: next.completed ? new Date() : null, activeKey: next.completed ? null : row.activeKey }).where(eq(interactiveSessions.id, sessionId));
        if (next.completed && pack.kind === "study") {
          const summary = summarizeLesson(lesson, next);
          await tx.insert(studySessions).values({ id: sessionId, disciplineId: pack.disciplineId, topicId: pack.topicId, activityType: "AULA_INTERATIVA", durationMinutes: Math.round(next.elapsedSeconds / 60), result: `${summary.independent}/${summary.total} sem ajuda · ${summary.assisted} com apoio`, note: `${lesson.title}. Reforçar: ${summary.reinforce.join(", ") || "verificar retenção outro dia"}.` }).onConflictDoNothing();
          if (pack.topicId) {
            await tx.update(topics).set({ status: summary.reinforce.length ? "REVISAR" : "ESTUDANDO", updatedAt: new Date() }).where(eq(topics.id, pack.topicId));
            const today = new Date().toISOString().slice(0, 10);
            await tx.update(reviews).set({ status: "CONCLUIDA", completedAt: new Date() }).where(and(eq(reviews.topicId, pack.topicId), eq(reviews.status, "PENDENTE"), lte(reviews.scheduledFor, today)));
            ...
            const [scheduled] = await tx.select({ id: reviews.id }).from(reviews).where(and(eq(reviews.topicId, pack.topicId), eq(reviews.status, "PENDENTE"), eq(reviews.scheduledFor, scheduledFor))).limit(1);
            if (!scheduled) await tx.insert(reviews).values({ disciplineId: pack.disciplineId, topicId: pack.topicId, scheduledFor });
          }
        }
      }
