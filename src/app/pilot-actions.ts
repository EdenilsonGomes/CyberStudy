...
export async function startPilot() {
  ...
  const [session] = await db.insert(interactiveSessions).values(withOwner(userId, {
...
export async function runPilotCommand(sessionId: string, command: LessonCommand) {
  ...
  try {
    const result = await db.transaction(async (tx) => {
      ...
      const [row] = await tx.select().from(interactiveSessions).where(owned(interactiveSessions, userId, and(eq(interactiveSessions.id, sessionId), eq(interactiveSessions.lessonKey, binaryPilot.id)))).for("update");
      ...
      if (next !== row.state) {
        await tx.update(interactiveSessions).set({ state: next, updatedAt: new Date(), completedAt: next.completed ? new Date() : null }).where(owned(interactiveSessions, userId, eq(interactiveSessions.id, sessionId)));
        if (next.completed) {
          const summary = summarizeLesson(binaryPilot, next);
          // Reuse the existing study history; PK guarantees one completion per session.
          await tx.insert(studySessions).values(withOwner(userId, { id: sessionId, activityType: "PILOTO_INTERATIVO",
