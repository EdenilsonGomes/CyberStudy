...
export default async function PilotPage({ searchParams }: { searchParams: Promise<{ sessao?: string }> }) {
  ...
  if (sessao) {
    ...
    const [row] = await db.select().from(interactiveSessions).where(owned(interactiveSessions, userId, and(eq(interactiveSessions.id, sessao), eq(interactiveSessions.lessonKey, binaryPilot.id), eq(interactiveSessions.contentVersion, binaryPilot.version)))).limit(1);
    if (!row) notFound();
