...
export default async function StartStudyPage({ searchParams }: { searchParams: Promise<{ topico?: string; aula?: string; disciplina?: string; diagnostico?: string; erro?: string; revisao?: string }> }) {
  await requireAuth();
  ...
  if (!target) notFound();
  if (!target.diagnostic && query.revisao !== "1") {
    ...
    if (step && trail.href.startsWith("/estudar/sessao/")) redirect(trail.href);
  }
  ...
  const errors: Record<string, string> = {
    ...
    AI_RATE_LIMIT: "O Mistral atingiu o limite temporário de requisições. Aguarde alguns segundos e tente novamente.",
    AI_QUOTA: "O limite de uso/créditos do Mistral foi atingido. Verifique o saldo ou o limite de gastos da conta Mistral.",
  };
  return <div className="lesson-focus mx-auto max-w-3xl"><section className="lesson-card card"><div className="lab-body">
    ...
    <span className="lab-tag">{query.revisao === "1" ? "Revisão · seu avanço será preservado" : target.diagnostic ? "Diagnóstico opcional" : "Aula interativa · com seu material"}</span>
