...
export default async function StartStudyPage({ searchParams }: { searchParams: Promise<{ topico?: string; aula?: string; disciplina?: string; diagnostico?: string; erro?: string }> }) {
  ...
  return <div className="lesson-focus mx-auto max-w-3xl"><section className="lesson-card card"><div className="lab-body">
    ...
    {query.erro && <p role="alert" className="lab-error">{errors[query.erro] || "Não foi possível abrir o estudo."}</p>}
    ...
    <p className="lab-save-note">O conteúdo é preparado uma vez e reaproveitado. Respostas e pistas ficam salvas no banco. Não feche a página durante a primeira preparação.</p>
  </div></section></div>;
}
