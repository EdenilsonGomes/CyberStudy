...
export default async function ProgressPage() {
  ...
  return <div className="mx-auto max-w-5xl space-y-6"><header><p className="eyebrow">Evolução que ajuda a decidir</p><h1 className="page-title">Progresso</h1></header>
    ...
    <section className="card p-5 md:p-6"><h2 className="section-title mb-5">Avanço por disciplina</h2><div className="space-y-4">{disciplineRows.map((discipline) => { const trail = disciplineTrails.get(discipline.id)!; const mastery = trail.total ? Math.round(trail.completed / trail.total * 100) : 0; return <Link href={`/disciplinas/${discipline.id}`} key={discipline.id} className="block rounded-xl bg-[var(--surface-2)] p-4"><div className="mb-2 flex min-w-0 justify-between gap-3"><span className="min-w-0 font-bold">{discipline.name}</span><strong>{mastery}%</strong></div><div className="progress"><span style={{ width: `${mastery}%`, background: discipline.color }}/></div></Link>; })}</div></section>
