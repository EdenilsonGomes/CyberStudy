...
export default async function ProgressPage() {
  ...
  const disciplineTrails = new Map(await Promise.all(disciplineRows.map(async d => [d.id, await loadTrail(d.id)] as const)));
  ...
  return <div className="mx-auto max-w-5xl space-y-6"><header><p className="eyebrow">Evolução que ajuda a decidir</p><h1 className="page-title">Progresso</h1></header>
    ...
    <Link className="btn btn-primary w-full" href="/semestre">Domínio por conceito e preparação do semestre</Link>
    <MaterialStudyProgress/><PilotProgress/>
    <section className="card p-5 md:p-6"><h2 className="section-title mb-5">Conteúdo percorrido por disciplina</h2><div className="space-y-4">{disciplineRows.map((discipline) => { const trail = disciplineTrails.get(discipline.id)!; const mastery = trail.total ? Math.round(trail.completed / trail.total * 100) : 0; return <Link href={`/disciplinas/${discipline.id}`} key={discipline.id} className="block rounded-xl bg-[var(--surface-2)] p-4"><div className="mb-2 flex min-w-0 justify-between gap-3"><span className="min-w-0 font-bold">{discipline.name}</span><strong>{mastery}%</strong></div><div className="progress"><span style={{ width: `${mastery}%`, background: discipline.color }}/></div></Link>; })}</div></section>
    ...
    <div className="grid gap-5 md:grid-cols-2"><section className="card p-5"><div className="mb-4 flex items-center gap-3"><span className="metric-icon text-[var(--success)]"><CheckCircle2 size={19}/></span><div><p className="eyebrow">Conceitos fortes</p><h2 className="section-title">Bons resultados anteriores</h2></div></div>{strongTopics.length ? <div className="space-y-2">{strongTopics.map((topic) => <div key={topic.id} className="concept-row"><span className="min-w-0"><strong className="block truncate text-sm">{topic.name}</strong><small className="muted">{disciplineById.get(topic.disciplineId)}</small></span><span className="badge">{topic.mastery}%</span></div>)}</div> : <p className="muted text-sm">Continue a trilha para formar seus primeiros pontos fortes.</p>}</section>
      <section className="card p-5"><div className="mb-4 flex items-center gap-3"><span className="metric-icon text-[var(--warning)]"><AlertTriangle size={19}/></span><div><p className="eyebrow">Conceitos fracos</p><h2 className="section-title">Vale reforçar</h2></div></div>{weakTopics.length ? <div className="space-y-2">{weakTopics.map((topic) => <Link key={topic.id} href={`/estudar?topico=${topic.id}&sessao=1&revisao=1`} className="concept-row"><span className="min-w-0"><strong className="block truncate text-sm">{topic.name}</strong><small className="muted">{disciplineById.get(topic.disciplineId)}</small></span><span className="badge">{topic.mastery}%</span></Link>)}</div> : <p className="muted text-sm">Nenhum ponto fraco identificado nas atividades recentes.</p>}</section></div>
  </div>;
}
