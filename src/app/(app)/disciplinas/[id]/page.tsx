...
import { disciplines, materials, topics } from "@/db/schema";
import { createTopic, deleteMaterial, deleteTopic, organizeMaterial, updateTopicStatus } from "@/app/actions";
...
export default async function DisciplinePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const completed = topicRows.filter((topic) => topic.status === "DOMINADO").length;
  const progress = topicRows.length ? Math.round((completed / topicRows.length) * 100) : 0;

  return <div className="mx-auto max-w-5xl space-y-6">
    <Link href="/disciplinas" className="muted flex items-center gap-2 text-sm"><ArrowLeft size={16}/>Disciplinas</Link>
    <header className="card overflow-hidden"><div className="h-2" style={{ background: discipline.color }}/><div className="p-5 md:p-8"><span className="badge mb-3">{discipline.semester}</span><h1 className="page-title">{discipline.name}</h1><p className="muted mt-2 max-w-2xl">{discipline.description || "Avance um assunto por vez."}</p><div className="mt-6"><div className="mb-2 flex justify-between text-sm"><span>{completed} de {topicRows.length} etapas concluídas</span><strong>{progress}%</strong></div><div className="progress"><span style={{ width: `${progress}%`, background: discipline.color }}/></div></div>{nextTopic && <Link className="btn btn-primary mt-6 w-full md:w-auto" href={`/estudar?topico=${nextTopic.id}&sessao=1`}><Play size={18} fill="currentColor"/>Continuar trilha</Link>}</div></header>
